...
(function () {
  ...
  const injectStyles = function () {
    ...
    style.textContent = `
      ...
      .logoType { position: relative; display:flex; flex-direction:column; align-items:flex-start; text-align:left; color:#fff; line-height:1.08; opacity: 0; transition: opacity 1200ms ease-in-out; font-family: var(--font-accent); }
      ...
      .nav-sour-widget span { position: relative; z-index: 2; }
      @keyframes popInWidget { 0% { transform: scale(0.5); opacity: 0; } 100% { transform: scale(1); opacity: 1; } }
      @media (max-width: 767px) {
        ...
        .nav-sour-widget { right: 3%; bottom: 4%; width: clamp(65px, 22vw, 95px); font-size: clamp(0.55rem, 2.2vw, 0.85rem); line-height: 1.25; animation: popInWidgetMobile 0.6s cubic-bezier(0.2, 0.8, 0.2, 1) forwards 2.6s; }
  ...
  const init = function () {
    ...
    root.innerHTML = `
      ...
      <div class="hero-frame">
        ...
        <a href="/sourapple/" target="_blank" class="nav-sour-widget">
          <span>海外品種<br>特設サイト</span>
        </a>
      </div>`;
